...
// Each call copies the existing map so sibling contexts do not share mutable state.
...
// GetStartTime returns the time recorded by SetStartTime.
...
// SetStartTime stores the RPC start time in ctx for interceptor use, such as latency measurement.
